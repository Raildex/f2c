...
std::string generate_include_guard(const char *in_file) {
  ...
  std::transform(include_guard.begin(),include_guard.end(), include_guard.begin(),[](unsigned char c) {return std::toupper(c);});
  return include_guard;
}

void convert_to_header(const char *in_file, const char *var_name,
                           const char *out_file, const char *name_space, const char* source_ext, const char* header_ext) {
  ...
  header_stream << "\textern const unsigned char " << var_name << "["
                << bytes.size() << "];\n";
...
int main(int argc, char **argv) {
  ...
  if (argc == 5) {
    if (!strcmp(argv[1], "-c")) {
      convert_to_header(argv[2], argv[3], argv[4], nullptr,"c", "h");
    }
  } else if (argc == 6) {
    ...
    if (!strcmp(argv[1], "-cpp")) {
      ...
      convert_to_header(argv[3], argv[4], argv[5], argv[2],"cpp", "hpp");
    }
  }
}