...
int main(int argc, char** argv) {
	...
	if(argc == 5) {
		if(!strcmp(argv[1], "-c")) {
			...
			auto converter = rdx::f2c::c_converter{.variable_name = argv[3], .bytes = rdx::f2c::read_file_to_byte_array(argv[2]), .out_file = out_file};
		...
		if(!strcmp(argv[1], "-cpp")) {
			std::filesystem::path out_file = argv[4];
			auto converter = rdx::f2c::cpp_converter{.variable_name = argv[3], .bytes = rdx::f2c::read_file_to_byte_array(argv[2]), .out_file = out_file , .name_space = argv[5]};
